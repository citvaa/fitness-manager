...
class UserServiceImplTest {
 @Mock UserRepository users; @Mock UserMapper mapper; @Mock PasswordEncoder encoder; @Mock AppConfig config; @Mock JwtUtil jwt; @Mock UserRoleRepository roles; @Mock TrainerRepository trainers; @Mock ClientRepository clients; @Mock PaymentRepository payments; @Mock EmailService email;
 UserServiceImpl service;
 @BeforeEach void setup(){service=new UserServiceImpl(users,mapper,encoder,config,jwt,roles,trainers,clients,payments,email);}
 ...
 @Test void registerActivatesOnlyValidKey(){User u=User.builder().registrationKeyValidity(LocalDateTime.now().plusMinutes(5)).build();when(users.findByRegistrationKey("key")).thenReturn(Optional.of(u));when(encoder.encode("pw")).thenReturn("hash");service.register(new RegisterUserRequest("key","pw"));assertTrue(u.getIsActivated());assertNull(u.getRegistrationKey());verify(users).save(u);}
 ...
 @Test void addAndRemoveRoleMutateOwnershipLink(){User u=User.builder().id(7).userRoles(new HashSet<>()).build();when(users.findById(7)).thenReturn(Optional.of(u));service.addRole(7,Role.TRAINER);assertEquals(1,u.getUserRoles().size());UserRole link=u.getUserRoles().iterator().next();service.removeRole(7,Role.TRAINER);verify(roles).save(any(UserRole.class));verify(roles).delete(link);assertTrue(u.getUserRoles().isEmpty());}
}
