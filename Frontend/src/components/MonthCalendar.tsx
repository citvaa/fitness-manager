...
export function MonthCalendar({value,onChange,highlightedDates,mutedDates=new Set(),mutedWeekdays=new Set()}:{value:string;onChange:(date:string)=>void;highlightedDates:Set<string>;mutedDates?:Set<string>;mutedWeekdays?:Set<number>}) {
  ...
  return <section className="month-calendar"><header><button type="button" aria-label="Prethodni mesec" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}>‹</button><strong>{month.toLocaleDateString('sr-Latn-RS',{month:'long',year:'numeric'})}</strong><button type="button" aria-label="Sledeći mesec" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}>›</button></header><div className="month-weekdays">{['Po','Ut','Sr','Če','Pe','Su','Ne'].map(day=><span key={day}>{day}</span>)}</div><div className="month-grid">{days.map((date,index)=>{if(!date)return <span key={`blank-${index}`}/>;const dateIso=iso(date);const muted=mutedDates.has(dateIso)||mutedWeekdays.has(date.getDay());return <button type="button" key={dateIso} aria-label={muted?`${date.getDate()}, teretana ne radi`:undefined} className={`${dateIso===value?'selected ':''}${highlightedDates.has(dateIso)?'highlighted ':''}${muted?'muted':''}`} onClick={()=>onChange(dateIso)}>{date.getDate()}<i/></button>})}</div></section>
}
