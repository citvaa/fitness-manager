...
export function NotificationCenter(){
  ...
  return <div className="notification-center"><label>Obaveštenja<select value={user?.notificationPreference??'PUSH'} onChange={e=>void change(e.target.value as UserAccount['notificationPreference'])}><option value="PUSH">Push</option><option value="EMAIL">Email</option><option value="BOTH">Oba</option></select></label><details><summary>🔔 {messages.length||''}</summary><div>{messages.length?messages.map((message,index)=><p key={index}>{message}</p>):<p>Nema novih obaveštenja.</p>}</div></details></div>
}
