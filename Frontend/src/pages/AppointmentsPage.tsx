...
export function AppointmentsPage() {
  ...
  const [closure, setClosure] = useState({mutedDates:new Set<string>(),mutedWeekdays:new Set<number>(),mutedDateReasons:new Map<string,string>(),mutedWeekdayReasons:new Map<number,string>()})
  ...
  const trainerReasons:Record<TrainerSchedule['status'],string>={WORKING:'',HOLIDAY:'Trener nema radnu smenu – praznik.',SICK_LEAVE:'Trener je nedostupan – bolovanje.',VACATION:'Trener je nedostupan – odmor.'}
  const mutedDateReasons=new Map<string,string>([
    ...schedules.filter(row=>row.status!=='WORKING').map(row=>[row.date,trainerReasons[row.status]||'Trener nema radnu smenu.'] as [string,string]),
    ...closure.mutedDateReasons,
  ])
