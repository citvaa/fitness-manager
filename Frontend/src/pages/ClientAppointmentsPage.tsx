...
export function ClientAppointmentsPage(){
  ...
  useEffect(()=>{appointmentsApi.mine().then(setAppointments).catch(cause=>setError(errorMessage(cause)))},[])
  ...
  return <main className="workspace-page appointments-page">
    ...
    <div className="calendar-list-layout"><MonthCalendar value={selectedDate} onChange={setSelectedDate} highlightedDates={new Set(appointments.map(item=>item.date))}/><div>
