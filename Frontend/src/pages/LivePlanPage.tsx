...
export function LivePlanPage(){
 const wrap=useRef<HTMLDivElement>(null);const [scale,setScale]=useState(1);const [gym,setGym]=useState<Gym|null>(null);const [rooms,setRooms]=useState<Room[]>([]);const [loading,setLoading]=useState(true);const [loadError,setLoadError]=useState('');const {snapshot,connected,error}=useOccupancy()
 ...
 useEffect(()=>{Promise.all([gymApi.gym(),gymApi.rooms()]).then(([g,r])=>{setGym(g);setRooms(r)}).catch(e=>setLoadError(errorMessage(e))).finally(()=>setLoading(false))},[])
