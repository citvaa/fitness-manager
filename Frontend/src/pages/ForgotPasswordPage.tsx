...
export function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Always show the same confirmation regardless of outcome, so this endpoint can't be used
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-slate-800 bg-slate-900/60 p-8 shadow-2xl shadow-black/40 backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-xl font-semibold text-slate-100">Zaboravljena lozinka</h1>
          <p className="mt-1 text-sm text-slate-400">Unesi svoj email da dobiješ link za reset</p>
        </div>

        {sent ? (
          <p className="rounded-lg bg-emerald-950/60 px-3 py-2 text-center text-sm text-emerald-300">
            Ako nalog sa ovim emailom postoji, poslat je link za reset lozinke.
          </p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {submitting ? 'Slanje...' : 'Pošalji link za reset'}
            </button>
          </form>
        )}
