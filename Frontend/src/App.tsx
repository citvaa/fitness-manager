...
const LivePlanPage = lazy(() =>
  import("./pages/LivePlanPage").then((m) => ({ default: m.LivePlanPage })),
);
const ManagerInsightsPage = lazy(() =>
  import("./pages/ManagerInsightsPage").then((m) => ({
    default: m.ManagerInsightsPage,
  })),
);
...
const AdministrationPage = lazy(() =>
  import("./pages/AdministrationPage").then((m) => ({
...
function ProtectedShell() {
  ...
  useEffect(() => {
    ...
    const refreshIn = Math.max(
      0,
      decodeJwt(session.accessToken).exp * 1000 - Date.now() - 60_000,
    );
...
function RoleHome() {
  ...
  return (
    <Navigate to={role === "MANAGER" ? "/app/live" : "/app/progress"} replace />
  );
}
function ManagerOnly({ children }: { children: React.ReactNode }) {
  return useAuthStore((s) => s.session?.activeRole) === "MANAGER" ? (
    children
  ) : (
    <Navigate to="/app/progress" replace />
  );
}
function ProgressOnly({ children }: { children: React.ReactNode }) {
  const role = useAuthStore((s) => s.session?.activeRole);
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={<AccountFlowPage mode="register" />}
      />
      <Route
        path="/forgot-password"
        element={<AccountFlowPage mode="forgot" />}
      />
      <Route
        path="/reset-password"
        element={<AccountFlowPage mode="reset" />}
      />
      <Route path="/app" element={<ProtectedShell />}>
        ...
        <Route
          ...
          element={
            <ManagerOnly>
              <Suspense fallback={loading("Učitavanje editora…")}>
                <EditorPage />
              </Suspense>
            </ManagerOnly>
          }
        ...
        <Route
          ...
          element={
            <ManagerOnly>
              <Suspense fallback={loading("Učitavanje uvida…")}>
                <ManagerInsightsPage />
              </Suspense>
            </ManagerOnly>
          }
        ...
        <Route
          ...
          element={
            <ManagerOnly>
              <Suspense fallback={loading("Učitavanje administracije…")}>
                <AdministrationPage />
              </Suspense>
            </ManagerOnly>
          }
        ...
        <Route
          path="schedules"
          element={
            <Suspense fallback={loading("Učitavanje rasporeda…")}>
              <SchedulesPage />
            </Suspense>
          }
        />
        ...
        <Route
          ...
          element={
            <ProgressOnly>
              <Suspense fallback={loading("Učitavanje napretka…")}>
                <ProgressPage />
              </Suspense>
            </ProgressOnly>
          }
